#!/usr/bin/env python3
"""
OddsMagnet Cricket Real-Time Collector
Continuously collects Cricket odds from all leagues
Updates oddsmagnet_cricket.json every 60 seconds
"""
...
class CricketRealtimeCollector:
    """Real-time collector for cricket odds"""
    
    # Important market categories for Cricket
    # NOTE: These are market CATEGORIES (not individual market names)
    IMPORTANT_MARKETS = [
        'popular markets',           # Contains: win market, handicaps, totals
        'match odds',                # Match winner markets
        'innings runs',              # Total runs in innings
        'top batsman',               # Top batsman markets
        'player performance'         # Player-specific markets
    ]
    ...
    def __init__(self, max_workers: int = 20, requests_per_second: float = 15.0):
        ...
        self.output_file = Path(__file__).parent / 'oddsmagnet_cricket.json'
        self.running = True
        self.iteration = 0
    ...
    def get_all_matches(self) -> List[Dict]:
        """Get all available cricket matches from OddsMagnet"""
        ...
        return temp_collector.get_all_matches_summary(sport='cricket', use_cache=True)
    
    def fetch_match_odds(self, match: Dict) -> Optional[Dict]:
        """Fetch odds for a single cricket match"""
        try:
    ...
    def run_realtime_loop(self, update_interval: float = 60.0, max_matches: int = 300):
        ...
        while self.running:
            ...
            # Step 1: Get all cricket matches
            print("\n📡 Fetching cricket match list...")
...
def main():
    """Main entry point"""
    print("\n" + "#"*80)
    print("ODDSMAGNET CRICKET REAL-TIME COLLECTOR")
    print("#"*80)
    
    print(f"\nImportant Markets ({len(CricketRealtimeCollector.IMPORTANT_MARKETS)}):")
    for i, market in enumerate(CricketRealtimeCollector.IMPORTANT_MARKETS, 1):
        print(f"  {i}. {market}")
    print()
    
    # Create collector with optimized settings
    collector = CricketRealtimeCollector(
