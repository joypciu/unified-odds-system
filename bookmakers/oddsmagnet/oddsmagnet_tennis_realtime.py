#!/usr/bin/env python3
"""
OddsMagnet Tennis Real-Time Collector
Continuously collects tennis odds from all leagues (Grand Slams, ATP, WTA, ITF, etc.)
Updates oddsmagnet_tennis.json every 30 seconds
"""
...
class TennisRealtimeCollector:
    """Real-time collector for tennis odds"""
    ...
    def __init__(self, max_workers: int = 20, requests_per_second: float = 15.0):
        ...
        self.output_file = Path(__file__).parent / 'oddsmagnet_tennis.json'
        self.running = True
        self.iteration = 0
    ...
    def get_all_matches(self) -> List[Dict]:
        """Get all available tennis matches from OddsMagnet"""
        ...
        return temp_collector.get_all_matches_summary(sport='tennis', use_cache=True)
    
    def fetch_match_odds(self, match: Dict) -> Optional[Dict]:
        """Fetch odds for a single tennis match with dynamic market discovery"""
        try:
    ...
    def run_realtime_loop(self, update_interval: float = 30.0):
        ...
        print("\n" + "="*80)
        print("REAL-TIME COLLECTION STARTED - TENNIS (ALL TOURNAMENTS)")
        print("="*80)
        ...
        while self.running:
            ...
            # Step 1: Get all tennis matches
            print("\n🎾 Fetching tennis match list...")
            ...
            if len(league_counts) > 10:
                print(f"  ... and {len(league_counts) - 10} more tournaments")
                ...
                for future in as_completed(future_to_match):
                    ...
                    try:
                        ...
                        if match_data:
                            ...
                            if completed % 50 == 0 or completed == len(all_matches):
            ...
            print(f"  🏆 Tournaments tracked: {len(league_counts)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='OddsMagnet Tennis Real-Time Collector')
    ...
    print("\n" + "="*80)
    print("ODDSMAGNET TENNIS COLLECTOR - DYNAMIC MARKET DISCOVERY")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    collector = TennisRealtimeCollector(
