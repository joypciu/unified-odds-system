#!/usr/bin/env python3
"""
OddsMagnet boxing Real-Time Collector
Continuously collects boxing odds from all leagues (MLB, NPB, KBO, etc.)
Updates oddsmagnet_boxing.json every 60 seconds
"""
...
class boxingRealtimeCollector:
    """Real-time collector for boxing odds"""
    ...
    def __init__(self, max_workers: int = 20, requests_per_second: float = 15.0):
        ...
        self.output_file = Path(__file__).parent / 'oddsmagnet_boxing.json'
        self.running = True
        self.iteration = 0
    ...
    def get_all_matches(self) -> List[Dict]:
        """Get all available boxing matches from OddsMagnet"""
        ...
        return temp_collector.get_all_matches_summary(sport='boxing', use_cache=True)
    
    def fetch_match_odds(self, match: Dict) -> Optional[Dict]:
        """Fetch odds for a single boxing match with dynamic market discovery"""
        try:
    ...
    def run_realtime_loop(self, update_interval: float = 30.0):
        ...
        print("\n" + "="*80)
        print("REAL-TIME COLLECTION STARTED - boxing (ALL LEAGUES)")
        print("="*80)
        ...
        while self.running:
            ...
            # Step 1: Get all boxing matches
            print("\n⚾ Fetching boxing match list...")
            all_matches = self.get_all_matches()
            
            if not all_matches:
                print("⚠️ No boxing matches found")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='OddsMagnet boxing Real-Time Collector')
    ...
    print("\n" + "="*80)
    print("ODDSMAGNET boxing COLLECTOR - DYNAMIC MARKET DISCOVERY")
    print("="*80)
    print(f"\nConfiguration:")
