#!/usr/bin/env python3
"""
OddsMagnet Table Tennis Real-Time Collector
Continuously collects table tennis odds from all leagues (Setka Cup, Czech Pro League, etc.)
Updates oddsmagnet_tabletennis.json every 30 seconds
"""
...
class TableTennisRealtimeCollector:
    """Real-time collector for table tennis odds"""
    ...
    def __init__(self, max_workers: int = 20, requests_per_second: float = 15.0):
        ...
        self.output_file = Path(__file__).parent / 'oddsmagnet_tabletennis.json'
        self.running = True
        self.iteration = 0
    ...
    def get_all_matches(self) -> List[Dict]:
        """Get all available table tennis matches from OddsMagnet"""
        ...
        return temp_collector.get_all_matches_summary(sport='table-tennis', use_cache=True)
    
    def fetch_match_odds(self, match: Dict) -> Optional[Dict]:
        """Fetch odds for a single table tennis match with dynamic market discovery"""
        try:
    ...
    def run_realtime_loop(self, update_interval: float = 30.0):
        ...
        print("\n" + "="*80)
        print("REAL-TIME COLLECTION STARTED - TABLE TENNIS (ALL LEAGUES)")
        print("="*80)
        ...
        while self.running:
            ...
            # Step 1: Get all table tennis matches
            print("\n🏓 Fetching table tennis match list...")
            all_matches = self.get_all_matches()
            
            if not all_matches:
                print("⚠️ No table tennis matches found")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='OddsMagnet Table Tennis Real-Time Collector')
    ...
    print("\n" + "="*80)
    print("ODDSMAGNET TABLE TENNIS COLLECTOR - DYNAMIC MARKET DISCOVERY")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    collector = TableTennisRealtimeCollector(
