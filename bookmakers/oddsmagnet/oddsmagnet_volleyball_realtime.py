#!/usr/bin/env python3
"""
OddsMagnet volleyball Real-Time Collector
Continuously collects volleyball odds from all leagues (MLB, NPB, KBO, etc.)
Updates oddsmagnet_volleyball.json every 60 seconds
"""
...
class volleyballRealtimeCollector:
    """Real-time collector for volleyball odds"""
    ...
    def __init__(self, max_workers: int = 20, requests_per_second: float = 15.0):
        ...
        self.output_file = Path(__file__).parent / 'oddsmagnet_volleyball.json'
        self.running = True
        self.iteration = 0
    ...
    def get_all_matches(self) -> List[Dict]:
        """Get all available volleyball matches from OddsMagnet"""
        ...
        return temp_collector.get_all_matches_summary(sport='volleyball', use_cache=True)
    
    def fetch_match_odds(self, match: Dict) -> Optional[Dict]:
        """Fetch odds for a single volleyball match with dynamic market discovery"""
        try:
    ...
    def run_realtime_loop(self, update_interval: float = 30.0):
        ...
        print("\n" + "="*80)
        print("REAL-TIME COLLECTION STARTED - volleyball (ALL LEAGUES)")
        print("="*80)
        ...
        while self.running:
            ...
            # Step 1: Get all volleyball matches
            print("\n⚾ Fetching volleyball match list...")
            all_matches = self.get_all_matches()
            
            if not all_matches:
                print("⚠️ No volleyball matches found")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='OddsMagnet volleyball Real-Time Collector')
    ...
    print("\n" + "="*80)
    print("ODDSMAGNET volleyball COLLECTOR - DYNAMIC MARKET DISCOVERY")
    print("="*80)
    print(f"\nConfiguration:")
