"""
NHL Data Collection - Multiple Markets
Collects NHL betting data from bet365 including:
...
class NHLParser:
    ...
    def __init__(self):
        self.all_games = {
            ...
            'alt_spreads': [],
            'alt_totals': [],
            'goalscorers': [],  # Changed from goal_scorers
            'goals': [],
            'assists': [],
            'points': [],
            'shots_on_goal': [],
    ...
    def parse_game_lines(self, body: str, all_captured_data: Optional[List[Dict]] = None) -> List[Dict]:
        """Parse game lines (spreads, totals, moneylines) from bet365 response

        Format:
        - PA;ID=PC37647892;NA=MIN Wild;N2=CHI Blackhawks;...;FI=185334675;
        - MA;ID=M972;NA=Line; (spreads)
        - PA;ID=37647892;FI=185334675;HD=-1.5;OD=2/1;
        - MA;ID=M1530;NA=Total; (totals)
        - PA;ID=37647894;FI=185334676;HD=O 5.5;OD=20/23;
        - MA;ID=M973;NA=Money Line; (moneylines)
        - PA;ID=37647903;FI=185334677;OD=10/13;

        Note: bet365 uses sequential fixture_ids for different markets of the same game
        - Base FI for game card and spreads
        - Base+1 FI for totals
        - Base+2 FI for moneylines
        """
        games = {}  # fixture_id -> game data
        sections = body.split('|')

        # First pass: Extract game cards and create entries for all related fixture_ids
        for section in sections:
            if section.startswith('PA;ID=PC') and ';FI=' in section:
                ...
                base_fixture_id = parts.get('FI', '')
                if base_fixture_id:
                    away_team = parts.get('NA', '')
                    home_team = parts.get('N2', '')
                    matchup = f"{away_team} @ {home_team}"

                    # Create game entries for base and related fixture_ids
                    for i, market_type in enumerate(['spreads', 'totals', 'moneylines']):
                        fixture_id = str(int(base_fixture_id) + i)
                        if fixture_id not in games:
                            games[fixture_id] = {
                                'fixture_id': base_fixture_id,  # Use base fixture_id for all
                                'matchup': matchup,
                                'away_team': away_team,
                                'home_team': home_team,
                                'betlink': f'https://www.on.bet365.ca/#/AC/B17/C20836572/D48/E{base_fixture_id}/F10/',
                                'spreads': [],
                                'totals': [],
                                'moneylines': []
                            }

        # Second pass: Extract odds by market type
        current_market = None

        for section in sections:
            # Identify market type
            if section.startswith('MA;ID=M'):
                parts = {}
                for item in section.split(';'):
                    if '=' in item:
                        key, val = item.split('=', 1)
                ...
                elif market_id == 'M1530':
                    current_market = 'total'
                elif market_id == 'M973':
                    current_market = 'moneyline'

            # Parse odds entries
            elif section.startswith('PA;ID=') and not section.startswith('PA;ID=PC') and ';FI=' in section:
                parts = {}
                for item in section.split(';'):
                    if '=' in item:
                        key, val = item.split('=', 1)
                ...
                if fixture_id in games and odds_raw:
                    # Find the base game for this fixture_id
                    base_game = None
                    for game in games.values():
                        if game['fixture_id'] == fixture_id or str(int(game['fixture_id']) + 1) == fixture_id or str(int(game['fixture_id']) + 2) == fixture_id:
                            base_game = game
                            break
                    ...
                    if base_game:
                        ...
                        if current_market == 'spread':
                            line = parts.get('HD', '')
                            if line:
                                base_game['spreads'].append({
                                    'line': line,
                                    'odds': odds_american,
                                    'odds_raw': odds_raw
                                })

                        elif current_market == 'total':
                            line = parts.get('HD', '')
                            if line:
                                # Extract just the number from "O 5.5" or "U 5.5"
                                line_type = 'Over' if line.startswith('O') else 'Under'
                                line_value = line.split()[-1] if ' ' in line else line[1:].strip()
                                base_game['totals'].append({
                                    'type': line_type,
                                    'line': line_value,
                                    'odds': odds_american,
                                    'odds_raw': odds_raw
                        ...
                        elif current_market == 'moneyline':
                            base_game['moneylines'].append({
                                'odds': odds_american,
                                'odds_raw': odds_raw
                            })

        # Return unique games (one per base fixture_id)
        unique_games = {}
        for game in games.values():
            base_id = game['fixture_id']
            if base_id not in unique_games:
                unique_games[base_id] = game
    ...
    def parse_response(self, body: str, market_name: str, market_url: str, all_captured_data: Optional[List[Dict]] = None) -> Dict:
        ...
        if market_name == 'Game Lines':
            result['game_lines'] = self.parse_game_lines(body, all_captured_data)
        elif market_name == 'Total':
            result['alt_totals'] = self._parse_totals(body)
        elif market_name == 'Goalscorers':
            result['goalscorers'] = self.parse_goalscorers(body)
            ...
            result['assists'] = self._parse_player_props(body, 'assists')
                            ...
                            'Winning Conference', 'Division of Stanley Cup Winner', 'Regular Season Points']:
            ...
            if 'MG;ID=G' in line and 'FI=' in line:
                ...
                if 'NA' in game_info and 'BC' in game_info:
                    ...
                    current_game = {
                        ...
                        'betlink': f'https://www.on.bet365.ca/#/AC/B17/C20836572/D48/E{fixture_id}/F10/'
                    }
    ...
    def _parse_player_props(self, body: str, prop_type: str) -> List[Dict]:
        """Parse player props (points, assists) from bet365 response
    ...
    def parse_goalscorers(self, body: str) -> List[Dict]:
        """Parse goalscorer markets from bet365 response

        Format:
        - PA;ID=P45709555;NA=Jason Robertson;N2=DAL Stars;TD=43;
        - MA;ID=C170348;NA=First;FI=185334736;
        - PA;ID=45709555;OD=9/1;SU=0;FI=185334736;
        - MA;ID=C170351;NA=Last;FI=185334736;
        - PA;ID=45709641;OD=9/1;SU=0;FI=185334736;
        - MA;ID=C170352;NA=Anytime;FI=185334736;
        - PA;ID=45709686;OD=7/5;SU=0;FI=185334736;
        """
        goalscorers = []
        sections = body.split('|')

        # Track players and markets
        players = {}  # player_id -> {name, team, fixture_id, first_odds, last_odds, anytime_odds}
        current_market = None  # 'First', 'Last', 'Anytime'

        for section in sections:
            # Player card: PA;ID=P45709555;NA=Jason Robertson;N2=DAL Stars;
            if section.startswith('PA;ID=P') and ';NA=' in section and ';N2=' in section:
                parts = {}
                for item in section.split(';'):
                    if '=' in item:
                        key, val = item.split('=', 1)
                ...
                player_id_full = parts.get('ID', '')
                if player_id_full.startswith('P'):
                    player_id = player_id_full[1:]  # Remove P prefix
                    players[player_id] = {
                        'player_name': parts.get('NA', ''),
                        'team_name': parts.get('N2', ''),
                        'first_odds': None,
            ...
            # Market identifier: MA;ID=C170348;NA=First;
            elif section.startswith('MA;ID=C1703'):
                parts = {}
                for item in section.split(';'):
                    if '=' in item:
                        key, val = item.split('=', 1)
                ...
                market_name = parts.get('NA', '')
                if market_name in ['First', 'Last', 'Anytime']:
                    current_market = market_name.lower()

            # Odds entry: PA;ID=45709555;OD=9/1;SU=0;
            elif section.startswith('PA;ID=') and not section.startswith('PA;ID=P') and ';OD=' in section and current_market:
                parts = {}
                for item in section.split(';'):
                    if '=' in item:
                        key, val = item.split('=', 1)
                ...
                if odds_raw:
                    ...
                    for player_id, player_data in players.items():
                        if participant_id.startswith(player_id) or participant_id == player_id:
                            ...
                            if current_market == 'first':
                                player_data['first_odds'] = odds_american
                            elif current_market == 'last':
                                player_data['last_odds'] = odds_american
                            elif current_market == 'anytime':
                                player_data['anytime_odds'] = odds_american
        ...
        # Convert to list
        for player_id, player_data in players.items():
            if any([player_data['first_odds'], player_data['last_odds'], player_data['anytime_odds']]):
                goalscorers.append({
                    'fixture_id': player_data.get('fixture_id', ''),
                    'player_name': player_data['player_name'],
                    'team_name': player_data['team_name'],
                    'first_goalscorer_odds': player_data['first_odds'],
                    'last_goalscorer_odds': player_data['last_odds'],
                    'anytime_goalscorer_odds': player_data['anytime_odds']
                })

        return goalscorers

    def parse_futures(self, body: str, market_name: str, betlink: str) -> List[Dict]:
        """Parse NHL futures markets (Stanley Cup winner, Conference, Division, etc.)"""
        futures = []
    ...
    def get_summary(self) -> str:
        ...
        goalscorers = len(self.all_games['goalscorers'])
        assists = len(self.all_games['assists'])
        points = len(self.all_games['points'])
        shots_on_goal = len(self.all_games['shots_on_goal'])
        blocked_shots = len(self.all_games['blocked_shots'])
        ...
        if goalscorers > 0:
            summary_parts.append(f"{goalscorers} Goalscorers")
        if assists > 0:
            summary_parts.append(f"{assists} Assists")
        if points > 0:
            summary_parts.append(f"{points} Points")
        if shots_on_goal > 0:
            summary_parts.append(f"{shots_on_goal} Shots on Goal")
        if blocked_shots > 0:
            summary_parts.append(f"{blocked_shots} Blocked Shots")
        if team_pts > 0:
    ...
    def save_futures_separately(self, filepath: str):
        """Save NHL futures data separately"""
        futures_data = {
...
async def collect_nhl_data():
    """NHL data collection with multiple markets"""
    import subprocess
    import shutil
    from pathlib import Path
    ...
    # Define markets to collect - NHL market codes (provided by user)
    # Market types and props:
    markets = [
        # Game lines (Main market with spreads, totals, moneylines)
        {'code': '972', 'name': 'Game Lines', 'hash': '#/AC/B17/C20836572/D48/E972/F10/', 'parser': 'game_lines', 
         'url': 'https://www.on.bet365.ca/#/AC/B17/C20836572/D48/E972/F10/', 'type': 'main'},
        
        # Player Props
        {'code': '170601', 'name': 'Points', 'hash': '#/AC/B17/C20836572/D43/E170601/F43/N0/', 'parser': 'player_props',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C20836572/D43/E170601/F43/N0/', 'type': 'props'},
        {'code': '170348', 'name': 'Goalscorers', 'hash': '#/AC/B17/C20836572/D43/E170348/F43/N0/', 'parser': 'player_props',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C20836572/D43/E170348/F43/N0/', 'type': 'props'},
        {'code': '170602', 'name': 'Assists', 'hash': '#/AC/B17/C20836572/D43/E170602/F43/N0/', 'parser': 'player_props',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C20836572/D43/E170602/F43/N0/', 'type': 'props'},
        {'code': '170240', 'name': 'Total', 'hash': '#/AC/B17/C20836572/D47/E170240/F47/N0/', 'parser': 'totals',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C20836572/D47/E170240/F47/N0/', 'type': 'totals'},
        
        # Futures
        {'code': '119212673', 'name': 'To Win Outright', 'hash': '#/AC/B17/C21061316/D1/E119212673/F2/', 'parser': 'futures',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C21061316/D1/E119212673/F2/', 'type': 'futures'},
        {'code': '120229690', 'name': 'To Win Conference', 'hash': '#/AC/B17/C21061316/D1/E120229690/F2/', 'parser': 'futures',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C21061316/D1/E120229690/F2/', 'type': 'futures'},
        {'code': '120229694', 'name': 'To Win Division', 'hash': '#/AC/B17/C21061316/D1/E120229694/F2/', 'parser': 'futures',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C21061316/D1/E120229694/F2/', 'type': 'futures'},
        {'code': '120358026', 'name': 'Winning Conference', 'hash': '#/AC/B17/C21061316/D1/E120358026/F2/', 'parser': 'futures',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C21061316/D1/E120358026/F2/', 'type': 'futures'},
        {'code': '120358023', 'name': 'Division of Stanley Cup Winner', 'hash': '#/AC/B17/C21061316/D1/E120358023/F2/', 'parser': 'futures',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C21061316/D1/E120358023/F2/', 'type': 'futures'},
        {'code': '121182525', 'name': 'Regular Season Points', 'hash': '#/AC/B17/C21061316/D1/E121182525/F2/', 'parser': 'futures',
         'url': 'https://www.on.bet365.ca/#/AC/B17/C21061316/D1/E121182525/F2/', 'type': 'futures'},
    ]
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            # Wait a bit more before clicking NHL
            logger.info("⏳ Waiting before clicking NHL...")
            await asyncio.sleep(random.uniform(1, 2))

            logger.info("🏒 Clicking on NHL...")
            # Click on NHL element
            nhl_selectors = [
                'span.wn-PillItem_Text:has-text("NHL")',
                '.wn-PillItem:has-text("NHL")',
                '[alt="NHL"]'
            ]

            nhl_clicked = False
            for selector in nhl_selectors:
                try:
                    element = await page.wait_for_selector(selector, timeout=5000)
                    if element:
                        await element.click()
                        logger.info(f"✓ Clicked NHL with: {selector}")
                        nhl_clicked = True
                        break
                except:
                    continue
            ...
            if not nhl_clicked:
                logger.warning("⚠️ Could not click NHL, navigating directly...")
                await page.goto('https://www.on.bet365.ca/#/AC/B17/C20836572/D48/E972/F10/')

            await asyncio.sleep(2)

            # Dynamic market discovery
            logger.info("🔍 Discovering available NHL markets...")
            available_markets = []
            ...
            # Use predefined markets for NHL since dynamic discovery may not find all markets
            ...
            logger.info(f"\n{'='*60}")
            logger.info(f"📊 Collecting {len(markets)} NHL Markets")
            logger.info(f"{'='*60}\n")
            ...
            output_dir = Path(f'nhl_data_{timestamp}')
            ...
            logger.info(f"\n{'='*60}")
            logger.info(f"✅ NHL Data Collection Complete")
            logger.info(f"{'='*60}")
